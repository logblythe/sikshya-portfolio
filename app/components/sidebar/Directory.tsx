...
function Directroy({ directory, callback }: IProps) {
  ...
  const defaultClasses =
    "w-full py-1 outline-none focus:shadow-item-selected focus:bg-white-10a focus:z-20 relative";
  const conditionalClasses =
    level === 0
      ? "bg-dark-500 font-bold uppercase"
      : "hover:bg-dark-400 font-medium";
        ...
        <>
          ...
          >
            <span className="flex items-center">
              <ChevronRightIcon
                className={`text-white-50a h-5 w-5 ${
          ...
          <Disclosure.Panel className="relative">
            <LevelContext.Provider value={level + 1}>
              {level > 0 && (
                <div
                  className="absolute z-30 border-l border-l-transparent h-full group-hover:border-l-dark-200 transition-colors"
